...
When `DATABASE_URL` is empty, PGlite persists data in `.data/postgres`. Restarting the server preserves data. Set `DATABASE_URL` to use PostgreSQL instead. New workspaces contain only the center and initial owner account. Sample records require explicit `SEED_DEMO=true` on a separate, empty development database. Startup never reseeds an existing center.
...
For the full browser workflow suite, use a separate development database with demo seeding explicitly enabled, then run:
...
The current deployment uses `SEED_DEMO=false`. To deploy another version from this linked checkout, run `railway up --service web`. The `.railwayignore` file excludes local credentials, local databases, research source files, generated requirement documents, and test output from uploads. Railway project configuration remains in `railway.json`.

## Current operating boundaries

The hosted workspace and default local workspace contain no preloaded students, families, inquiries, schedules, or attendance. It has no connection to Kumon corporate systems, payment processing, or outbound email and SMS. Guardian contact history records staff-entered notes.
