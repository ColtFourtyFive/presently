...
export default function ReportsPage({ data, onStudent }: PageProps) {
  ...
  return <>
    ...
    <section className="card"><div className="card-header"><div><h2>Attendance records <span className="count-pill">{visits.length}</span></h2><p className="muted">Visits are included by their check-in date in your center's time zone.</p></div></div>{visits.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Student</th><th>Date</th><th>Check-in</th><th>Check-out</th><th>Duration</th><th>Status</th></tr></thead><tbody>{visits.map(visit => { const student = data.students.find(item => item.id === visit.studentId); return <tr key={visit.id}><td><button className="student-name-button" disabled={!student} onClick={() => student && onStudent(student)}><Avatar name={student ? fullName(student) : 'Student'} size="sm" /><strong>{student ? fullName(student) : 'Unknown student'}</strong></button></td><td>{dateLabel(visit.checkedInAt, data.center.timezone)}</td><td>{shortTime(visit.checkedInAt, data.center.timezone)}</td><td>{visit.checkedOutAt ? shortTime(visit.checkedOutAt, data.center.timezone) : <span className="muted">{visit.reconciliationStatus === 'review_needed' ? 'Presence unverified' : 'Still checked in'}</span>}</td><td>{visit.reconciliationStatus === 'review_needed' ? <span className="muted">Needs review</span> : visit.checkedOutAt ? `${Math.max(0, Math.round((new Date(visit.checkedOutAt).getTime() - new Date(visit.checkedInAt).getTime()) / 60000))} min` : '—'}</td><td><Badge tone={visit.reconciliationStatus === 'review_needed' ? 'amber' : visit.status === 'open' ? 'blue' : 'green'}>{visit.reconciliationStatus === 'review_needed' ? 'Needs review' : visit.status === 'open' ? 'At center' : 'Completed'}</Badge></td></tr>; })}</tbody></table></div> : <EmptyState icon={CalendarCheck2} title="No records to show" description="Adjust the date range or record a student's first check-in." />}</section>
  </>;
}
