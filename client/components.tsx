...
export function SubjectTags({subjects}:{subjects:Subject[]}) { return <span className="subject-tags">{subjects.map(s=><span className={`subject-tag ${s==='Math'?'subject-math':'subject-reading'}`} key={s}>{s==='Math'?<span className="subject-symbol">∑</span>:<span className="subject-symbol">Aa</span>}{s}</span>)}</span>; }
...
export function Modal({title,subtitle,children,onClose,wide=false}:{title:string;subtitle?:string;children:ReactNode;onClose:()=>void;wide?:boolean}) {
  ...
  useEffect(()=>{
    ...
    const first=ref.current?.querySelector<HTMLElement>('input:not([type="checkbox"]),select,textarea')||ref.current?.querySelector<HTMLElement>('button');first?.focus();
