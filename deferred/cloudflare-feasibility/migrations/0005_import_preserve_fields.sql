-- Read the saved column mapping, not normalized blank values, so pending previews
-- created by older Workers also preserve fields that were not supplied.
...
WHEN OLD.status='pending' AND NEW.status='applied' BEGIN
 ...
 SELECT OLD.student_id,i.center_id,json_extract(OLD.payload_json,'$.studentCode'),json_extract(OLD.payload_json,'$.firstName'),json_extract(OLD.payload_json,'$.lastName'),1,
   coalesce(json_extract(OLD.payload_json,'$.subjects'),'[]'),coalesce(json_extract(OLD.payload_json,'$.pickupAlert'),''),
