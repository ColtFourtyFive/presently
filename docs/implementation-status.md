...
On September 14, 2026, `npm test` passed 23 tests across three files. The suite covers authentication and session scope, background polling, role restrictions, attendance retries and concurrency, pickup permissions, exceptional and unmatched departures, correction provenance, inquiry conversion, and report export authorization. It also verifies that the fictional demo initializes successfully and does not duplicate data on restart, and that a new nested local database directory starts successfully and preserves committed data after reopening. Tests use ephemeral PGlite databases without external services or real student data.
...
The deployed preview uses synthetic data and does not complete the remaining operational controls above. A future operational environment should use a separate empty database with demo seeding disabled.
